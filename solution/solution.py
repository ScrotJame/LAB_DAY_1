...
def compare_models(prompt: str) -> dict:
    ...
    mini_pricing = PRICING_PER_1K_TOKENS.get(
        OPENAI_MINI_MODEL, PRICING_PER_1K_TOKENS["gpt-4o"]
    )
...
def call_openai_with_system(
    ...
    latency = max(time.perf_counter() - start_time, 1e-6)
    response_text = response.choices[0].message.content

    return response_text, latency


# Alias cho test — test gọi chat_with_system_prompt
chat_with_system_prompt = call_openai_with_system
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Giữ tối đa 4 lượt cuối (8 message)
        history = history[-8:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<43} | {'GPT-4o Response':<43} | {'Mini Response':<43} | {'GPT-4o Latency':>14} | {'Mini Latency':>12}"
    sep = "-" * len(header)
    rows = [header, sep]
    ...
    for r in results:
        row = (
            ...
            f"{truncate(r.get('gpt4o_answer', '')):<43} | "
            f"{truncate(r.get('mini_answer', '')):<43} | "
            f"{r.get('gpt4o_time', 0):>13.2f}s | "
            f"{r.get('mini_time', 0):>11.2f}s"
        )
        rows.append(row)
